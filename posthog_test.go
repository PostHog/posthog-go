...
func TestEnqueue_UUID_PreSupplied(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Capture",
			msg:  Capture{Uuid: "f47ac10b-58cc-11cf-a567-0e02b2c3d479", Event: "test", DistinctId: "user1"},
			uuid: "f47ac10b-58cc-11cf-a567-0e02b2c3d479",
		},
		{
			name: "Alias",
			msg:  Alias{Uuid: "f47ac10b-58cc-31cf-a567-0e02b2c3d479", Alias: "new", DistinctId: "user1"},
			uuid: "f47ac10b-58cc-31cf-a567-0e02b2c3d479",
		},
		{
			name: "Identify",
		...
		{
			name: "GroupIdentify",
			msg:  GroupIdentify{Uuid: "f47ac10b-58cc-51cf-a567-0e02b2c3d479", Type: "company", Key: "acme"},
			uuid: "f47ac10b-58cc-51cf-a567-0e02b2c3d479",
		},
		{
			name: "Exception",
			msg: Exception{
				Uuid:       "01890fdb-39d5-7d72-a7dc-1b022b2f7111",
				DistinctId: "user1",
				ExceptionList: []ExceptionItem{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Read the body sent to the server
			select {
			case res := <-body:
...
func TestEnqueue_UUID_InvalidPreSupplied(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			select {
			case res := <-body:
				gotUUID := requireValidUUIDInBatch(t, res)
				require.NotEqual(t, tt.invalidUUID, gotUUID,
...
// TestEnqueue_UUID_AutoGenerated verifies that UUIDs are auto-generated when
// not provided by the user for all message types.
func TestEnqueue_UUID_AutoGenerated(t *testing.T) {
	tests := []struct {
		name string
		msg  Message
	}{
		{
			name: "Capture",
			msg:  Capture{Event: "test", DistinctId: "user1"},
		},
		{
			name: "Alias",
			msg:  Alias{Alias: "new", DistinctId: "user1"},
		},
		{
			name: "Identify",
			msg:  Identify{DistinctId: "user1"},
		},
		{
			name: "GroupIdentify",
			msg:  GroupIdentify{Type: "company", Key: "acme"},
		},
		{
			name: "Exception",
			msg: Exception{
...
func requireValidUUIDInBatch(t *testing.T, body []byte) string {
	t.Helper()
	...
	require.NoError(t, uuid.Validate(payload.Batch[0].Uuid))

	return payload.Batch[0].Uuid
}
...
func TestCaptureMany(t *testing.T) {
	...
	// UUIDs for the first batch (3 events) - remaining 2 events will be in a second batch
	uuids := []string{
