...
func TestEnqueue_UUID_AutoGenerated(t *testing.T) {
	// UUID v7 format regex: 8-4-4-4-12 hex characters with version nibble 7.
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			case res := <-body:
				...
				matches := uuidPattern.FindAllString(resStr, -1)
				require.NotEmpty(t, matches,
					"auto-generated UUID should be present in request body")
				// Verify it's not empty string
				require.NotContains(t, resStr, `"uuid":""`,
